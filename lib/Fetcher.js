...
const Dataset = require('indexed-dataset/dataset')
...
class Fetcher {
  ...
  static fetchDataset (options) {
    ...
    return rdfFetch(options.url, options.options)
      .then((res) => {
        ...
        return new Dataset().import(res.quadStream())
      })
  }
  ...
  static load (dataset, options) {
    ...
    return Fetcher.fetchDataset(options).then((input) => Fetcher.spreadDataset(input, dataset, options))
  }
}
